...
<!-- When Anki is unreachable, new cards queue here:
...
- **The pending queue holds cards created while Anki was unreachable.** Flush it at the next session where the server is reachable — add the cards, move them into the provenance table, clear the queue.
- **One deck per workspace.** The deck is `{Topic}` — a top-level deck named after the topic, matching the mission. Never group topics under a shared parent deck. If the user wants a different name, record it here and use it consistently.
